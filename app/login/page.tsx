...
function LoginContent() {
  ...
  return (
    <main className="bento-login min-h-screen bg-[#0b1020] flex items-center justify-center p-5 sm:p-8 relative overflow-hidden">
      ...
      >
        <div aria-hidden="true" className="mx-auto mb-7 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 shadow-lg shadow-indigo-900/40">
        ...
        {error && (
          <div role="alert" className="mt-6 rounded-xl border border-rose-400/30 bg-rose-400/10 p-3 text-sm text-rose-100">{error}</div>
        )}
        ...
        >
          {loading ? <><span aria-hidden="true" className="h-5 w-5 animate-spin rounded-full border-2 border-slate-400 border-t-slate-900" />Signing in…</> : <><span aria-hidden="true" className="grid h-5 w-5 place-items-center rounded-full bg-white text-sm font-bold">G</span>Continue with Google</>}
        </button>
